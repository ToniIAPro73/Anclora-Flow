...
const BACKEND_URL = process.env.BACKEND_URL || `http://localhost:${process.env.PORT || 8020}`;
...
async function dispatchVerificationEmail(user, token) {
    ...
    <div style="font-family: 'Inter', Arial, sans-serif; max-width: 512px; margin: 0 auto; background: #f8fafc; padding: 32px;">
      <div style="background: linear-gradient(135deg, #3366ff 0%, #14b8a6 50%, #d946ef 100%); padding: 24px; border-radius: 18px;">
        ...
        <p style="color: rgba(255,255,255,0.92); margin: 12px 0 0; font-size: 15px;">
          Hola ${user.first_name || user.firstName || user.name || 'creador/a'}, estamos casi listos.
        </p>
      </div>
  ...
  const text = `Hola ${user.first_name || user.firstName || user.name || 'creador/a'}, confirma tu correo en Anclora Flow: ${link}`;

  await sendEmail({ to: user.email, subject, html, text });
}

async function dispatchPasswordResetEmail(user, token) {
