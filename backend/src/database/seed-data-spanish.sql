...
INSERT INTO budgets (id, user_id, client_id, category, month, planned_amount, actual_amount, notes, budget_number, title, description, subtotal, total, status, valid_until, created_at, updated_at) VALUES
((SELECT gen_random_uuid()), (SELECT id FROM users WHERE email = 'pmi140979@gmail.com'), (SELECT id FROM clients WHERE business_name = 'Fernández Arquitectura SA'), 'development', DATE_TRUNC('month', NOW())::date, 12000.00, 0, NULL, 'PPTO-2026-001', 'Portal de Clientes Arquitectura', 'Desarrollo de portal web para gestión de proyectos arquitectónicos con visualización de planos', 12000.00, 12720.00, 'sent', NOW() + INTERVAL '30 days', NOW() - INTERVAL '15 days', NOW()),
((SELECT gen_random_uuid()), (SELECT id FROM users WHERE email = 'pmi140979@gmail.com'), (SELECT id FROM clients WHERE business_name = 'Torres Logística SL'), 'logistics', DATE_TRUNC('month', NOW())::date, 18500.00, 0, NULL, 'PPTO-2026-002', 'App Móvil Tracking Avanzado', 'Aplicación móvil con geolocalización en tiempo real y notificaciones push', 18500.00, 19610.00, 'draft', NOW() + INTERVAL '45 days', NOW() - INTERVAL '5 days', NOW()),
((SELECT gen_random_uuid()), (SELECT id FROM users WHERE email = 'pmi140979@gmail.com'), (SELECT id FROM clients WHERE business_name = 'Ecommerce Solutions España'), 'ecommerce', DATE_TRUNC('month', NOW())::date, 8900.00, 0, NULL, 'PPTO-2025-003', 'Rediseño Completo Tienda Online', 'Renovación UI/UX completa con nueva arquitectura de información', 8900.00, 9434.00, 'accepted', NOW() + INTERVAL '60 days', NOW() - INTERVAL '20 days', NOW());
