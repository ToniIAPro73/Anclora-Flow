...
class Budget {
  static normalizeMonth(month?: string | Date): string {
    if (!month) {
  ...
  static async findByMonth(userId: string, month?: string | Date): Promise<IBudget[]> {
    ...
    return result.rows.map(row => ({
      ...
      relatedRevenue: parseFloat(row.related_revenue),
      remainingAmount: parseFloat(row.remaining_amount),
      spendingRatio: parseFloat(row.spending_ratio)
    }));
  }
  ...
  static async getSummary(userId: string, month?: string | Date): Promise<IBudgetSummary> {
    ...
    return {
      plannedTotal: parseFloat(row.planned_total) || 0,
      actualTotal: parseFloat(row.actual_total) || 0,
      remainingTotal: parseFloat(row.remaining_total) || 0,
      onTrackCategories: parseInt(row.on_track_categories) || 0,
      trackedCategories: parseInt(row.tracked_categories) || 0,
    };
  }
  ...
  static async getSuggestions(userId: string, month?: string | Date, monthsLookback: number = 3): Promise<IBudgetSuggestion[]> {
    ...
    return result.rows.map(row => ({
      category: row.category,
      avgMonthlySpend: parseFloat(row.avg_monthly_spend),
      peakSpend: parseFloat(row.peak_spend),
      plannedAmount: parseFloat(row.planned_amount),
      suggestedDelta: parseFloat(row.suggested_delta),
  ...
  static async update(userId: string, id: string, payload: IBudgetUpdate): Promise<IBudget> {
    const fields: string[] = [];
    const values: any[] = [];
    let index = 1;
