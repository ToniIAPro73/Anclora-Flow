...
class Subscription {
  ...
  static async getSummary(userId: string): Promise<ISubscriptionSummary | null> {
    ...
    return {
      ...
      pausedSubscriptions: parseInt(row.paused_subscriptions),
      cancelledSubscriptions: parseInt(row.cancelled_subscriptions),
      totalAmount: parseFloat(row.total_amount),
      monthlyRecurringRevenue: parseFloat(row.monthly_recurring_revenue),
      next30DaysRevenue: parseFloat(row.next_30_days_revenue)
    };
  }
